...
class UPS {
    ...
    save = async id => {
        await new Builder(`tbl_stocks_info`)
            .insert({ columns: `stocks_id, serial_no, model, output_capacity`, 
                            values: `${id}, ${(this.data).serial_no !== '' ? `'${((this.data).serial_no).toUpperCase()}'` : null}, ${(this.data).model !== '' ? `'${((this.data).model).toUpperCase()}'` : null}, 
                                            ${(this.data).output_capacity !== '' ? `'${((this.data).output_capacity).toUpperCase()}'` : null}` })
            .build();
    }
    ...
    update = async prev => {
        ...
        if(Global.compare(prev.output_capacity, curr.output_capacity)) {
            audits.push({ series_no: Global.randomizer(7), table_name: 'tbl_stocks', item_id: prev.id, field: 'Output capacity', previous: prev.output_capacity,
                current: curr.output_capacity !== '' && curr.output_capacity !== null ? (curr.output_capacity).toUpperCase() : null, action: 'update', user_id: user.id, date: date });
        }
        ...
        if(!(errors.length > 0)) {
            ...
            await new Builder(`tbl_stocks_info`)
                .update(`serial_no= ${curr.serial_no !== '' && curr.serial_no !== null ? `'${(curr.serial_no).toUpperCase()}'` : null}, 
                                model= ${curr.model !== '' && curr.model !== null ? `'${(curr.model).toUpperCase()}'` : null}, 
                                output_capacity= ${curr.output_capacity !== '' && curr.output_capacity !== null ? `'${(curr.output_capacity).toUpperCase()}'` : null}`)
