                            ...
                            { iss.action === 'update' ? 
                                <Typography color= "#636e72">
                                    { `${(iss.action).charAt(0).toUpperCase() + (iss.action).slice(1)} ${(iss.field).replace('_', ' ')} of 
                                        ${(iss.model ?? iss.serial_no).toUpperCase()} from '${iss.previous !== null ? (iss.previous).replace('_', ' ') : ''}'
                                        to '${iss.current !== null ? (iss.current).replace('_', ' ') : ''}'.` }</Typography> : 
                                <Typography color= "#636e72">
                                    { `${(iss.model ?? iss.serial_no).toUpperCase()} ${(iss.action).toLowerCase()}d.` }</Typography> }
