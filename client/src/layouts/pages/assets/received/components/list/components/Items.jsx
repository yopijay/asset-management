...
const Items = () => {
    ...
    let authview = data.user_level === 'superadmin' || (data.permission === null || JSON.parse(data.permission).assets.received.view);
                        ...
                        { list.map((iss, index) => 
                            <Stack direction= "column" justifyContent= "flex-start" alignItems= "stretch" sx= { listview } key= { index } spacing= { 4 }>
                                <Stack direction= "column" justifyContent= "flex-start" alignItems= "stretch" sx= {{ flexGrow: 1, overflow: 'hidden' }} spacing= { 2 }>
                                    ...
                                    <Stack direction= "column" justifyContent= "flex-start" alignItems= "stretch">
                                        { data.user_level !== 'user' ? <Typography sx= { title }>Issued to: { iss.issued_to }</Typography> : '' }
                                ...
                                <Stack direction= "row" justifyContent= "flex-end" alignItems= "center" spacing= { 1 } paddingLeft= "10px">
                                    ...
                                    <Stack direction= "row" justifyContent= "flex-start" alignItems= "center" spacing= { .5 }>
                                        { (authupdate && iss.status === 'received') ? <Typography sx= { menu } component= { Link } to= { `/assets/received/form/update/${iss.id}` }>
                                                <FontAwesomeIcon icon= { solid('pencil') } />
                                            </Typography> : '' }
                                        { authview ? <Typography sx= { menu } component= { Link } to= { `/assets/received/form/view/${iss.id}` }>
                        ...
                        { list.map((iss, index) => 
                            <Grid item xs= { 12 } sm= { 6 } md= { 4 } key= { index }>
                                <Stack direction= "column" justifyContent= "flex-start" alignItems= "stretch" sx= { listview } 
                                    component= { (authupdate && iss.status === 'received') ? Link : Stack } to= { `/assets/received/form/update/${iss.id}` } spacing= { 4 }>
                                    <Stack direction= "column" justifyContent= "flex-start" alignItems= "stretch" sx= {{ flexGrow: 1, overflow: 'hidden' }} spacing= { 2 }>
                                        <Stack direction= "column" justifyContent= "flex-start" alignItems= "stretch">
                                            <Typography variant= "caption" sx= { caption }>{ iss.series_no }</Typography>
                                            <Typography variant= "body2" sx= { subtitle }>{ iss.category }</Typography>
