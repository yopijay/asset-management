...
import Classification from "../../classification"; // Fields
...
const Index = () => {
    ...
    const { isFetching, refetch } = 
        useGet({ key: ['asc_specific'], request: specific({ table: 'tbl_assets_supplies_classification', id: id ?? null }), options: { enabled: type !== 'new', refetchOnWindowFocus: false },
            onSuccess: data => {
    ...
    const { mutate: saving } = 
        usePost({ request: save,
            onSuccess: data => {
                ...
                else { successToast(data.message, 3000, navigate('/assets-supplies/classification', { replace: true })); }
            } 
        });
    ...
    return (
        <Stack sx= { content } spacing= { 4 }>
            <Stack direction= "column" justifyContent= "flex-start" alignItems= "stretch">
                <Typography sx= { title }>{ type.charAt(0).toUpperCase() + type.slice(1) } Classification</Typography>
            ...
            <Stack sx= { card }><FormBuilder fields= { Classification({ fetching: isFetching }) } /></Stack>
            <Stack direction= "row" justifyContent= "flex-end" alignItems= "center" spacing= { 1 }>
                <Typography sx= { cancelbtn } component= { Link } to= "/assets-supplies/classification">Cancel</Typography>
                { type !== 'view' ? <Typography sx= { savebtn } onClick= { handleSubmit(data => {
                    ...
                    if(!(errors.length > 0)) {
                        if(type === 'new') { saving({ table: 'tbl_assets_supplies_classification', data: data }); }
                        else { updating({ table: 'tbl_assets_supplies_classification', data: data }); }
                    }
