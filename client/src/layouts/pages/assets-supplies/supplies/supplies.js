...
const Supplies = ({ fetching }) => {
    ...
    const { data: types, isFetching: typsfetching } = 
        useGet({ key: ['typs_dd'], request: dropdown({ table: 'tbl_brands', data: { type: 'per-category', category: 'supplies' } }), options: { refetchOnWindowFocus: false } });
    const { data: brand, mutate: brdmenu, isLoading: brdloading } = usePost({ request: dropdown });
    useGet({ key: ['supp_series'], request: series('tbl_supplies'), options: {}, 
