        ...
        <Stack direction= "row" justifyContent= "flex-end" alignItems= "center" spacing= { 1 }>
            ...
            { sort === 'desc' ? 
                <Typography sx= {{ cursor: 'pointer' }} 
                    onClick= { () => { setsort('asc'); setValue('sort', 'asc'); refetch({ table: 'tbl_module', data: getValues() }); } }>
                    <FontAwesomeIcon icon= { solid('arrow-down-z-a') } color= "#9DB2BF" /></Typography> :
                <Typography sx= {{ cursor: 'pointer' }} 
                    onClick= { () => { setsort('desc'); setValue('sort', 'desc'); refetch({ table: 'tbl_module', data: getValues() }); } }>
