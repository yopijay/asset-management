...
const Logs = () => {
    const { data: log, isFetching: fetching } = useGet({ key: ['emp_logs'], request: history({ table: 'tbl_users', data: {} }), options: { refetchOnWindowFocus: false } }); 
                ...
                log?.length > 0 ? 
                    log?.map((data, index) => (
                        <Stack direction= "column" justifyContent= "flex-start" alignItems= "stretch" padding= "10px 12px" spacing= { 1 } key= { index }>
                            ...
                            { data.action === 'update' ? 
                                <Typography color= "#526D82">{ (data.action).toUpperCase() } { (data.field).toUpperCase() } OF { data.employee_no } FROM { data.previous } TO { data.current }</Typography> :
                                <Typography color= "#526D82">{ (data.action).toUpperCase() } { data.employee_no }</Typography>}
                        </Stack>
