...
const Search = props => {
    ...
    const { mutate: xlsx } = usePost({ request: excel, onSuccess: data => exporttoexcel(data, 'Position', `Position Logs-${today}`) });
                ...
                <Box sx= { search }>
                    <TextField { ...register('logssearchtxt') } variant= "standard" size= "small" fullWidth InputProps= {{ disableUnderline: true, 
                        ...
                        onChange= { e => { 
                            ...
                            find({ table: 'tbl_position', data: data });
                        } } />
                </Box>
            </form>
            <Stack direction= "row" justifyContent= "flex-end" alignItems= "center" spacing= { .5 }>
                <Typography sx= { download }
                    onClick= { () => {
                        ...
                        data['token'] = (sessionStorage.getItem('token')).split('.')[1];
                        data['type'] = 'logs';
                        xlsx({ table: 'tbl_position', data: data });
