...
const Items = () => {
    ...
    let authupdate = data.user_level === 'superadmin' || (data.permission === null || JSON.parse(data.permission).maintenance.brands.update);
    let authview = data.user_level === 'superadmin' || (data.permission === null || JSON.parse(data.permission).maintenance.brands.view);
                            ...
                            <Stack direction= "row" justifyContent= "space-between" alignItems= "flex-start" sx= { listview } key= { index } spacing= { 4 }>
                                ...
                                <Stack direction= "row" justifyContent= "flex-start" alignItems= "center" spacing= { 1 } paddingLeft= "10px">
                                    ...
                                    <Stack direction= "row" justifyContent= "flex-start" alignItems= "center" spacing= { .5 }>
                                        { authupdate ? <Typography sx= { menu } component= { Link } to= { `/maintenance/brands/form/update/${data.id}` }>
                                                <FontAwesomeIcon icon= { solid('pencil') } />
                                            </Typography> : '' }
                                        { authview ? <Typography sx= { menu } component= { Link } to= { `/maintenance/brands/form/view/${data.id}` }>
                        ...
                        { list.map((data, index) => 
                            <Grid item xs= { 12 } sm= { 6 } md= { 4 } key= { index }>
                                <Stack direction= "row" justifyContent= "space-between" alignItems= "flex-start" sx= { listview } 
                                    component= { authupdate ? Link : Stack } to= { `/maintenance/brands/form/update/${data.id}` } spacing= { 4 }>
                                    <Stack direction= "column" justifyContent= "flex-start" alignItems= "stretch" sx= {{ flexGrow: 1, overflow: 'hidden' }}>
                                        <Typography variant= "caption" sx= { caption }>{ data.series_no }</Typography>
                                        <Typography sx= { title }>{ data.name }</Typography>
                                        <Typography sx= { subtitle } variant= "body2">{ (data.category).toUpperCase() }</Typography>
